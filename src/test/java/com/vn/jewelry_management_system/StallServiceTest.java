...
public class StallServiceTest {
    ...
    public void testFindByStallName() {
        Stall stall = new Stall();
        when(stallRepository.findByStallName("TestStall")).thenReturn(Optional.of(stall));

        Optional<Stall> result = stallService.findByStallName("TestStall");
        assertTrue(result.isPresent());
        assertEquals(stall, result.get());
        verify(stallRepository, times(1)).findByStallName("TestStall");
    }
}