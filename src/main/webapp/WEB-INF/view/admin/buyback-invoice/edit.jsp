<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    <%@ taglib prefix="form" uri="http://www.springframework.org/tags/form" %>
        <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
            ...
            <head>
                <meta charset="UTF-8">
                <title>Edit Buyback Invoice</title>
            ...
            <body>
                <div class="container mt-5">
                    <h1>Edit Buyback Invoice</h1>
                    <form:form method="POST" action="/admin/buyback-invoices/edit/${buybackInvoice.buybackInvoiceId}"
                        modelAttribute="buybackInvoice">
                        <div class="mb-3">
                            ...
                            <form:select path="customer" class="form-select">
                                ...
                                <c:forEach items="${customers}" var="customer">
                                    <form:option value="${customer.customerId}"
                                        selected="${buybackInvoice.customer.customerId == customer.customerId}">
                                        ${customer.customerName}</form:option>
                                </c:forEach>
                            </form:select>
                        </div>
                        <div class="mb-3">
                            ...
                            <form:select path="employee" class="form-select">
                                ...
                                <c:forEach items="${employees}" var="employee">
                                    <form:option value="${employee.employeeId}"
                                        selected="${buybackInvoice.employee.employeeId == employee.employeeId}">
                                        ${employee.employeeName}</form:option>
                                </c:forEach>
                            </form:select>
                        </div>
                        <div class="mb-3">
                            ...
                            <form:select path="stall" class="form-select">
                                ...
                                <c:forEach items="${stalls}" var="stall">
                                    <form:option value="${stall.stallId}"
                                        selected="${buybackInvoice.stall.stallId == stall.stallId}">
                                        ${stall.stallName}</form:option>
                                </c:forEach>
                            </form:select>
                        </div>
                        <div class="mb-3">
